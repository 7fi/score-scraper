...
for i, regatta in enumerate(list(regattas.values())):
    ...
    for i in range(1, teamCount):
        ...
        teamScores["A"] = [int(scoreData[(i*3) - 3].contents[j].text) for j in range(4, (4 + raceCount)) if scoreData[(i*3) - 3].contents[j].text.isdigit()]
        teamScores["B"] = [int(scoreData[(i*3) - 2].contents[j].text) for j in range(4, (4 + raceCount)) if scoreData[(i*3) - 2].contents[j].text.isdigit()]

        teamNameEl = [i for i in sailors.find_all('td', class_="teamname") if i.text == teamName][0]
        ...
        while row.next_sibling is not None and row['class'][0] != "topborder" and row['class'][0] != "reserves-row" or index == 0:
            ...
            if skipperName != "":
                ...
                addPerson(skipper.text.split(" '")[0],"Skipper", division, teamHome, raceNums, teamScores[division], teamHomes, betterVenue)
            ...
            if crewName != "":
                ...
                raceNums = [i.split("-", 1) for i in raceNums]
                addPerson(crew.text.split(" '")[0],"Crew", division, teamHome, raceNums, teamScores[division], teamHomes, betterVenue)
